...
/* Private variables ---------------------------------------------------------*/
CAN_HandleTypeDef hcan1;    //CAN handler
/* USER CODE BEGIN PV */
uint8_t RxData[4];    //array for receiving the data
CAN_RxHeaderTypeDef   RxHeader;   //structure variable
/* USER CODE END PV */
...
void DeferredTask(void *argument)  //definition of deferred task
{
	while(1)
	{
		SEGGER_SYSVIEW_PrintfHost("Waiting for mque receive\n");  //to print in the terminal of segger systemview software
		ulTaskNotifyTake(pdTRUE, portMAX_DELAY);  //notifies the task about updation of resources
		//xQueueReceive(can1_BytesReceived, &QxData, portMAX_DELAY); 
		SEGGER_SYSVIEW_PrintfHost("Data: %d\n",RxData[0]); //to print in the terminal of segger systemview software
		SEGGER_SYSVIEW_PrintfHost("Data: %d\n",RxData[1]); //to print in the terminal of segger systemview software
		HAL_GPIO_WritePin(GPIOD, GPIO_PIN_12,1);  //gpio_writepin api to turn the green led on
		vTaskDelay(100/portTICK_PERIOD_MS);  //delay
		HAL_GPIO_WritePin(GPIOD, GPIO_PIN_12,0);  //gpio_writepin api to turn the green led off
		vTaskDelay(100/portTICK_PERIOD_MS);  //delay
	}
}

void WasteFullLED(void *argument)  //wasteful task
{
	while(1)
	{
		HAL_GPIO_WritePin(GPIOD, GPIO_PIN_15,1);  //gpio_writepin api to turn the blue led on
		vTaskDelay(100/portTICK_PERIOD_MS);  //delay
		HAL_GPIO_WritePin(GPIOD, GPIO_PIN_15,0);  //gpio_writepin api to turn the blue led off
		vTaskDelay(100/portTICK_PERIOD_MS);  //delay
	}
}
